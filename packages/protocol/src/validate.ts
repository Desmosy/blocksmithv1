...
function checkString(ctx: Ctx, v: unknown, path: string, minLength = 0): v is string {
...
function checkBlock(ctx: Ctx, b: unknown, path: string): b is BlocksmithBlockV1 {
  if (!isObject(b)) {
    ctx.fail(path, "block must be an object");
    return false;
  }
  if (checkString(ctx, b.id, `${path}.id`, 1) && !ID_RE.test(b.id as string)) {
    ctx.fail(`${path}.id`, `id "${b.id}" must match ${ID_RE}`);
  }
  if (!BLOCK_TYPES.has(b.type as string)) {
  ...
  } else if (b.source.ingest != null && !INGESTS.has(b.source.ingest as string)) {
  ...
  if (
    checkString(ctx, b.contentHash, `${path}.contentHash`, 1) &&
    !HASH_RE.test(b.contentHash as string)
  ) {
    ctx.fail(`${path}.contentHash`, "must match ^sha256:[0-9a-f]+$");
  }
  if (b.editedBy != null && !EDIT_ORIGINS.has(b.editedBy as string)) {
...
export function validateGraph(
  ...
  if (typeof input.systemId !== "string") ctx.fail("$.systemId", "expected string");
  if (
    checkString(ctx, input.contentHash, "$.contentHash", 1) &&
    !HASH_RE.test(input.contentHash as string)
  ) {
  ...
  input.blocks.forEach((b, i) => {
    ...
    if (isObject(b) && typeof b.id === "string") {
      if (seen.has(b.id)) ctx.fail(`$.blocks[${i}].id`, `duplicate id "${b.id}"`);
      seen.add(b.id);
      if (
      ...
      if (options.verifyHashes && isObject(b.content)) {
        const expected = blockContentHash(
          b.id as string,
          b.type as string,
          b.content,
        );
        if (b.contentHash !== expected) {
          ctx.fail(
            `$.blocks[${i}].contentHash`,
            `hash mismatch: got ${b.contentHash}, canonical is ${expected}`,
          );
        }
      }
    }
  ...
  if (options.verifyHashes && ctx.errors.length === 0) {
    const expected = graphHash(
      (input.blocks as BlocksmithBlockV1[]).map((b) => ({
        id: b.id,
        version: b.version,
    ...
    if (input.contentHash !== expected) {
      ctx.fail(
        "$.contentHash",
        `graph hash mismatch: got ${input.contentHash}, canonical is ${expected}`,
      );
    }
  }
...
export function validateLock(input: unknown): ValidationResult {
  const ctx = new Ctx();
  if (!isObject(input)) {
...
export function validateRegistryEntry(input: unknown): ValidationResult {
  const ctx = new Ctx();
  if (!isObject(input)) {
    ctx.fail("$", "entry must be an object");
    return ctx.result();
  }
  if (checkString(ctx, input.id, "$.id", 1) && !ID_RE.test(input.id as string)) {
  ...
  input.versions.forEach((v, i) => {
    ...
    if (!BLOCK_TYPES.has(v.type as string)) {
  ...
  if (input.official != null) {
    const versions = (input.versions as { version?: number }[]).map(
      (v) => v.version,
    );
...
export function validateRegistryManifest(input: unknown): ValidationResult {
  const ctx = new Ctx();
  if (!isObject(input)) {
  ...
  if (
    typeof input.officialGraphHash !== "string" ||
...
export function validateCompileTargets(input: unknown): ValidationResult {
  const ctx = new Ctx();
  if (!isObject(input)) {
    ctx.fail("$", "manifest must be an object");
    return ctx.result();
 ...
 */
export function verifyLockAgainstGraph(
  ...
  for (const b of graph.blocks) {
    ...
    if (pin.version !== b.version) {
      ctx.fail(
        `$.blocks["${b.id}"].version`,
        `locked v${pin.version}, graph has v${b.version}`,
      );
    } else if (pin.contentHash !== b.contentHash) {
      ctx.fail(
        `$.blocks["${b.id}"].contentHash`,
        "contentHash mismatch — lock corrupted or hand-edited",
      );
    }
  }
  ...
  if (stale) {
    ctx.fail(
      "$.contentHash",
      `lock is STALE: pinned ${lock.contentHash}, graph is ${expected}`,
    );
  }
...
export function lockFromGraph(
  graph: BlocksmithGraphV1,
  options?: { packageName?: string },
): BlocksmithLockV1 {
  const blocks: BlocksmithLockV1["blocks"] = {};

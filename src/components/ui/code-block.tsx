...
function buildCommand(pm: string, url: string) {
  switch (pm) {
    case "npm":  return `npm install -g ${url}`;
    case "yarn": return `yarn global add ${url}`;
    case "bun":  return `bun add -g ${url}`;
    default:     return `pnpm add -g ${url}`;
  }
}
...
interface InstallCommandProps {
  ...
  className?: string;
  isGlobal?: boolean;
  commandOverride?: (pm: string, url: string) => string;
}

export function InstallCommand({ registryUrl, className, isGlobal, commandOverride }: InstallCommandProps) {
  const [pm, setPm] = useState<string>("npm");
  const [copied, setCopied] = useState(false);

  const defaultBuilder = (pm: string, url: string) => {
    if (isGlobal) {
      switch (pm) {
        case "npm":  return `npm install -g ${url}`;
        case "yarn": return `yarn global add ${url}`;
  ...
  const command = commandOverride ? commandOverride(pm, registryUrl) : defaultBuilder(pm, registryUrl);
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      {/* Command row */}
      <div className="flex h-10 items-center justify-between gap-2 rounded-lg border bg-[var(--dash-surface)] px-3">

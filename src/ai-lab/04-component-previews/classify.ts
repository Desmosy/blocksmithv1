...
/** Map parsed component prose to a generic preview template (no brand lists). */
export function classifyComponentKind(component: ComponentDoc): ComponentPreviewKind {
  const titleRole = `${component.title} ${component.role}`.toLowerCase();
  const text = corpus(component);

  const fromTitle = (): ComponentPreviewKind | null => {
